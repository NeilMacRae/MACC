/**
 * InitiativesTab — Extracted from MACCPage.tsx for the MACC Modelling hub.
 *
 * Contains: D3 MACC chart, TargetOverlay, SuggestionRequest, AcceptModal,
 * DismissModal, InitiativeTable, InitiativeDetail, CreationChoiceModal,
 * EditInitiativeModal (renamed from inline EditModal).
 *
 * Data dependencies: useInitiatives, useEmissions, useSuggestions
 */
...
import { StatusTransition } from '../initiatives/StatusTransition';
import { SuggestionRequest } from '../suggestions/SuggestionRequest';
import { SuggestionCard } from '../suggestions/SuggestionCard';
import { AcceptModal } from '../suggestions/AcceptModal';
import { DismissModal } from '../suggestions/DismissModal';
...
function EditInitiativeModal({
  ...
  if (isLoading || !initiative)
    return (
      <Modal open title="Edit Initiative" onClose={onClose}>
  ...
  return (
    <div className="space-y-4">
      ...
      <div>
        ...
        <StatusTransition initiative={initiative} onTransitioned={onTransitioned} />
      </div>
      ...
      {initiative.notes && (
        <div>
          ...
          <p className="text-xs text-gray-500 whitespace-pre-wrap">{initiative.notes}</p>
        </div>
      )}
    </div>
  );
...
function CreationChoiceModal({
  ...
  return (
    <Modal open title="New Initiative" onClose={onClose}>
...
function AISuggestionsModal({
  ...
  const modalTitle = showingRequest
    ? 'AI-Powered Suggestions'
    : `AI-Powered Suggestions — ${suggestionResponse.suggestions.length} suggestions generated`;

  return (
    <>
      <Modal open title={modalTitle} onClose={onClose}>
        {showingRequest ? (
          <SuggestionRequest
              ...
              <div className="rounded-lg bg-gray-50 border border-gray-200 p-4">
                ...
                <div className="grid grid-cols-3 gap-4 text-xs">
                  ...
                  <div>
                    ...
                    <span className="font-medium text-gray-900">
                      {suggestionResponse.context_used.total_emissions_co2e.toLocaleString(
                        'en-GB',
            ...
            {suggestionResponse.constraints_relaxed && (
              <div className="rounded-lg bg-amber-50 border border-amber-200 p-4">
                <div className="flex gap-3">
                  <svg
                    className="flex-shrink-0 h-5 w-5 text-amber-600"
                    fill="none"
                    viewBox="0 0 24 24"
                    stroke="currentColor"
                  >
            ...
            {suggestionResponse.suggestions.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-12 text-center">
                ...
                <p className="text-sm text-gray-500 mt-1">
                  Try adjusting your request parameters
                </p>
                <div className="mt-4">
                  <Button onClick={() => setSuggestionResponse(null)}>
                    New Request
                  </Button>
                </div>
              </div>
              ...
              <>
                ...
                <div className="flex justify-center pt-4">
                  <Button variant="secondary" onClick={() => setSuggestionResponse(null)}>
  ...
  return (
    <div className="space-y-6 p-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-lg font-semibold text-gray-900">MACC Chart</h2>
          <p className="mt-0.5 text-sm text-gray-500">
            Marginal Abatement Cost Curve — initiatives sorted by cost per tonne of CO₂e
            reduced
          </p>
        </div>
        <Button onClick={() => setShowCreationChoice(true)}>
          + Add initiative
        </Button>
      </div>
      ...
      <div className="rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
        <h2 className="mb-4 text-base font-semibold text-gray-800">Abatement Curve</h2>
        {maccLoading && (
        ...
        {maccData && (
          <MACCChart data={maccData} selectedId={selectedId} onSelect={setSelectedId} />
        )}
      </div>
      ...
      <div className="grid gap-6 xl:grid-cols-5">
        <div className="xl:col-span-3 rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
          <h2 className="mb-4 text-base font-semibold text-gray-800">All Initiatives</h2>
          <InitiativeTable
        ...
        <div className="xl:col-span-2 rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
          {selectedInitiative ? (
            <InitiativeDetail
              ...
              onTransitioned={(updated) => setSelectedId(updated.id)}
            />
          ) : (
            <div className="flex h-full min-h-[200px] flex-col items-center justify-center text-center text-sm text-gray-600">
              <svg
              ...
              <p className="mt-1 text-gray-300">Click a bar or row to see details</p>
            </div>
          )}
        </div>
      </div>
      ...
      {showAI && (
        <AISuggestionsModal
          ...
          onInitiativeCreated={(id) => setSelectedId(id)}
        />
      )}
